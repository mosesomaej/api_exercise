...
# @app.get("/people/person/{person_name}", response_model=SchemaP)
# def get_person_by_name(person_name: str):
#     person = db.session.query(ModelP).filter(ModelP.Name == person_name).first()
#     if person is None:
#         raise HTTPException(
#         status_code=404,
#         detail= f"person with name: {person_name} does not exist"
#         )
#     return person
